...
public class main_activity extends AppCompatActivity {
    ...
    public static JSONArray dataConfig = new JSONArray();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //同步配置
        Button synConfig = findViewById(R.id.config);
        synConfig.setOnClickListener(v ->{
            OkHttpClient okhttp_client = public_func.get_okhttp_obj();
            Request request;
            try {
                JSONArray array = new JSONArray();
                JSONObject reqData = new JSONObject();
                reqData.put("name","reqData");
                JSONObject data = new JSONObject();
                reqData.put("data",data);
                data.put("g_schema_key","SMS_CONFIG");
                data.put("linesPerPage","100");
                data.put("currentPage","1");
                data.put("queryType","list");
                array.put(reqData);

                String url = "http://www.sunsharing.com.cn/projectManager/scurd/remote.action";
//                JSONObject post = new JSONObject();
//                post.put("service","scurdServiceImpl");
//                post.put("method","search");
//                post.put("params",array);
                String post = "service=scurdServiceImpl&method=search&params=" + URLEncoder.encode(array.toString(),"UTF-8");
                RequestBody body = RequestBody.create(post.toString(), MediaType.parse("application/x-www-form-urlencoded; charset=UTF-8"));
                request = new Request.Builder().url(url).method("POST", body).build();
            }catch (Exception e){
                Snackbar.make(v, Objects.requireNonNull(e.getMessage()), Snackbar.LENGTH_LONG)
                        .show();
                return;
            ...
            call.enqueue(new Callback() {
                ...
                public void onResponse(@NonNull Call call, @NonNull Response response) throws IOException {
                    ...
                    if (response.code() != 200) {
                        assert response.body() != null;
                        String result = Objects.requireNonNull(response.body()).string();
                        String error_message = "Send Message API Error:" + result;
                        public_func.write_log(context, error_message);
                        Snackbar.make(v, error_message, Snackbar.LENGTH_LONG).show();
                        return;
                    }
                    try {
                        ...
                        JsonArray array = result_obj.getAsJsonObject("data").getAsJsonObject("data").getAsJsonArray("list");

                        SharedPreferences.Editor editor = sharedPreferences.edit();
                        editor.putString("config", array.toString().trim());
                        editor.apply();

                        Snackbar.make(v, R.string.success + "同步关键字："+array.size()+"条", Snackbar.LENGTH_LONG)
                                .show();
                        public_func.stop_all_service(context);
                        public_func.start_service(context, true);
                    }catch (Exception e){
                        e.printStackTrace();
                    }
                    Looper.loop();
